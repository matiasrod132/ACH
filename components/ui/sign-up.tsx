...
import { ArrowRight, Mail, Gem, Lock, Eye, EyeOff, ArrowLeft, X, AlertCircle, PartyPopper, Loader, User as UserIcon } from "lucide-react";
...
// Real auth — replaces the original demo's fake setTimeout success flow.
...
function BlurFade({ children, className, variant, duration = 0.4, delay = 0, yOffset = 6, inView = true, inViewMargin = "-50px", blur = "6px" }: BlurFadeProps) {
  ...
  // `margin` narrows to framer-motion's MarginType (a template-literal union) in this
  // version; keeping the prop as a plain string (as originally authored) needs a cast.
  const inViewResult = useInView(ref, { once: true, margin: inViewMargin as Parameters<typeof useInView>[1] extends { margin?: infer M } ? M : never });
...
interface AuthComponentProps {
  ...
  brandName?: string;
  /** Short one-line description of what the app does, shown under the brand
   * mark. Present in the initial server-rendered HTML (no client JS needed
   * to see it) so the homepage always explains the app's purpose. */
  tagline?: string;
}
...
function friendlyAuthError(err: unknown): string {
  ...
  switch (code) {
    ...
    case 'auth/cancelled-popup-request':
      return '';
    default:
      return err instanceof Error ? err.message : 'Algo salió mal. Intentá de nuevo.';
  }
}

export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize", tagline }: AuthComponentProps) => {
  const { signIn, register, signInWithGoogle } = useGame();
  ...
  const [authAction, setAuthAction] = useState<'login' | 'register' | 'google'>('login');
  ...
  const submitAuth = async () => {
    ...
    try {
        ...
        if (!message) { setModalStatus('closed'); return; }
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'register' && authStep === 'confirmPassword') {
        void submitAuth();
    } else if (mode === 'login' && authStep === 'password' && isPasswordValid) {
        void submitAuth();
    }
  };
  ...
  const handleProgressStep = () => {
    if (authStep === 'email') {
        if (isEmailValid) setAuthStep(mode === 'register' ? 'name' : 'password');
  ...
  const handleGoBack = () => {
    ...
    else if (authStep === 'password') setAuthStep(mode === 'register' ? 'name' : 'email');
...
useEffect(() => {
  ...
  const loadingText = authAction === 'register' ? 'Creando tu cuenta...' : authAction === 'google' ? 'Conectando con Google...' : 'Iniciando sesión...';
  const successText = authAction === 'register' ? '¡Cuenta creada!' : '¡Bienvenido de nuevo!';
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card/80 border-4 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
                    ...
                    {modalStatus === 'error' && <>
                        ...
                        <GlassButton onClick={closeModal} size="sm" className="mt-4">Reintentar</GlassButton>
                    </>}
                    {modalStatus === 'loading' &&
  ...
  return (
    <div className="bg-background min-h-screen w-screen flex flex-col">
        ...
        <div className={cn( "fixed top-4 left-4 z-20 flex flex-col gap-0.5", "md:left-1/2 md:-translate-x-1/2 md:items-center" )}>
            <div className="flex items-center gap-2">
                {logo}
                <h1 className="text-base font-bold text-foreground">{brandName}</h1>
            </div>
            {tagline && <p className="max-w-[240px] text-xs text-muted-foreground md:text-center">{tagline}</p>}
        </div>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight text-foreground whitespace-nowrap">{mode === 'register' ? 'Creá tu cuenta' : 'Bienvenido de nuevo'}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 2}><p className="text-sm font-medium text-muted-foreground">Continuá con</p></BlurFade>
                        ...
                        <BlurFade delay={0.25 * 4} className="w-[300px]"><div className="flex items-center w-full gap-2 py-2"><hr className="w-full border-border"/><span className="text-xs font-semibold text-muted-foreground">O</span><hr className="w-full border-border"/></div></BlurFade>
                    </motion.div>}
                    {authStep === "name" && <motion.div key="name-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">¿Cómo te llamás?</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">Lo usamos para tu perfil dentro de {brandName}.</p></BlurFade>
                    </motion.div>}
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">{mode === 'register' ? 'Creá tu contraseña' : 'Ingresá tu contraseña'}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">{mode === 'register' ? 'Debe tener al menos 6 caracteres.' : `Iniciá sesión para continuar en ${brandName}.`}</p></BlurFade>
                    </motion.div>}
                     {authStep === "confirmPassword" && <motion.div key="confirm-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                         <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">Un último paso</p></div></BlurFade>
                         <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">Confirmá tu contraseña para continuar</p></BlurFade>
                    </motion.div>}
                </AnimatePresence>
                ...
                <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                        ...
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                            <BlurFade delay={authStep === 'email' ? 0.25 * 5 : 0} inView={true} className="w-full">
                                <div className="relative w-full">
                                    <AnimatePresence>
                                        {(authStep === "password" || authStep === "name") && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.4 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-muted-foreground font-semibold">Email</label></motion.div>}
                                    </AnimatePresence>
                            ...
                            <AnimatePresence>
                                {authStep === "name" && <BlurFade key="name-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {name.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-muted-foreground font-semibold">Nombre</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2"><UserIcon className="h-5 w-5 text-foreground/80 flex-shrink-0" /></div>
                                            <input ref={nameInputRef} type="text" placeholder="Nombre" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none" />
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isNameValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label="Continuar" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                        </div></div>
                                    </div>
                                    <BlurFade inView delay={0.2}><button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground transition-colors"><ArrowLeft className="w-4 h-4" /> Volver</button></BlurFade>
                                </BlurFade>}
                            </AnimatePresence>
                            <AnimatePresence>
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {password.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-muted-foreground font-semibold">Contraseña</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                                {isPasswordValid ? <button type="button" aria-label="Mostrar u ocultar contraseña" onClick={() => setShowPassword(!showPassword)} className="text-foreground/80 hover:text-foreground transition-colors p-2 rounded-full">{showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button> : <Lock className="h-5 w-5 text-foreground/80 flex-shrink-0" />}
                                            </div>
                                            <input ref={passwordInputRef} type={showPassword ? "text" : "password"} placeholder="Contraseña" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none" />
                    ...
                    <AnimatePresence>
                        {authStep === 'confirmPassword' && <BlurFade key="confirm-password-field" className="w-full">
                            <div className="relative w-full">
                                <AnimatePresence>
                                    {confirmPassword.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-muted-foreground font-semibold">Confirmar contraseña</label></motion.div>}
                                </AnimatePresence>
                                <div className="glass-input-wrap w-[300px]"><div className="glass-input">
                                    <span className="glass-input-text-area"></span>
                                    <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
                                        {isConfirmPasswordValid ? <button type="button" aria-label="Mostrar u ocultar confirmación de contraseña" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="text-foreground/80 hover:text-foreground transition-colors p-2 rounded-full">{showConfirmPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button> : <Lock className="h-5 w-5 text-foreground/80 flex-shrink-0" />}
                                    </div>
                                    <input ref={confirmPasswordInputRef} type={showConfirmPassword ? "text" : "password"} placeholder="Confirmar contraseña" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none" />
                                    <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isConfirmPasswordValid ? "w-10 pr-1" : "w-0" )}><GlassButton type="submit" size="icon" aria-label="Crear cuenta" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                </div></div>
                            </div>
                            <BlurFade inView delay={0.2}><button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground transition-colors"><ArrowLeft className="w-4 h-4" /> Volver</button></BlurFade>
                        </BlurFade>}
